...
#
# # 原始字符串
# s = "echo:Print time:  10y 1h 21m 40s"
#
# # 方法1: 使用正则表达式提取所有数字
# numbers = re.findall(r'\d+', s)
# print("所有数字:", numbers)  # 输出: ['10', '1', '21', '40']
#
# # 方法2: 转换为整数列表
# numbers_int = [int(num) for num in re.findall(r'\d+', s)]
# print("整数列表:", numbers_int)  # 输出: [10, 1, 21, 40]
#
# # 方法3: 提取特定格式的时间数字（带单位）
# time_components = re.findall(r'(\d+)y|(\d+)h|(\d+)m|(\d+)s', s)
# print("时间组件:", time_components)  # 输出: [('10', '', '', ''), ('', '1', '', ''), ('', '', '21', ''), ('', '', '', '40')]
#
# # 方法4: 提取并关联时间单位
# time_dict = {}
# for match in re.finditer(r'(\d+)([yhmds])', s):
#     value, unit = match.groups()
#     time_dict[unit] = int(value)

# line ='X:248.5760 Y:503.:6.3000 E:2621.6211 Count X:87002 Y:176366 Z:12600 STA: 5579519'
# x = line.split("X:")[1].split("Y")[0]
# y = line.split("Y:")[1].split("Z")[0]
# z = line.split("Z:")[1].split("E")[0]
# pos_str="X:" + x + "    Y:" + y + "    Z:" + z
# pos_str=pos_str
# if "STA" in line:
#     a = line.split("STA:")[1].replace("\n", "")
#     a = int(a)
